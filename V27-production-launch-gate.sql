...
begin
  ...
  r := r || jsonb_build_array(jsonb_build_object('workflow_runs','status',case when to_regclass('public.workflow_runs_v21') is not null or to_regclass('public.workflow_runs_v20') is not null then 'PASS' else 'WARN' end));
